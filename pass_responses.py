...
Original file is located at
    https://colab.research.google.com/drive/1Kq2bqVD11zOOrLdJ5ISoHNp2WmMC3nzu
...
*   COALESCE(product_type, 'Unknown'): If the product_type value is NULL, it is replaced with 'Unknown'

2.   **brand**


*   COALESCE(NULLIF(REPLACE(brand, '-', ''), ''), 'Unknown'):
* REPLACE(brand, '-', ''): Removes dashes from the brand name.
* NULLIF(..., ''): If the brand name becomes an empty string, it is converted to NULL.
* COALESCE(..., 'Unknown'): If the brand is NULL, it is replaced with 'Unknown'.

3. **weight**

* CAST(REGEXP_REPLACE(weight, '[^\d.]', '', 'g') AS DECIMAL(10, 2)): Removes all non-digit and non-decimal point characters from the weight column and converts the result to a decimal number.
* ROUND(..., 2): Rounds the result to two decimal places.

4. **price**

* TO_CHAR(CAST(price AS DECIMAL(10, 2)), '9999999999.99'): Converts the price column to a decimal number and formats it as a string in the specified format.
* If the price is NULL, it calculates the median value using SELECT PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY price) FROM products and formats it similarly.

5. **average_units_sold**

* COALESCE(average_units_sold, 0): If average_units_sold is NULL, it is replaced with 0.

6. **year_added**

* COALESCE(year_added, 2022): If year_added is NULL, it is replaced with 2022.

7. **stock_location**

* COALESCE(UPPER(stock_location), 'Unknown'): If stock_location is NULL, it is replaced with 'Unknown'. Additionally, it converts the stock location to uppercase.
